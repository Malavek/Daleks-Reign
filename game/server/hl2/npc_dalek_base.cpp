...
ConVar ai_readinessdalek_decay( "ai_readinessdalek_decay", "80" );// How many seconds it takes to relax completely
ConVar ai_new_aimingdalek( "ai_new_aimingdalek", "3" );
static ConVar scene_showfaceto("scene_showfaceto", "0", FCVAR_ARCHIVE, "When playing back, show the directions of faceto events.");

#define GetReadinessUse()	ai_use_readinessdalek.GetInt()

extern ConVar g_debug_transitions;
ConVar ai_debug_looktargetsdalek("ai_debug_looktargetsdalek", "0");
...
BEGIN_DATADESC( CNPC_Dalek_Base )
...
CNPC_Dalek_Base::eCoverType CNPC_Dalek_Base::gm_fCoverSearchType;
bool CNPC_Dalek_Base::gm_bFindingCoverFromAllEnemies;
string_t CNPC_Dalek_Base::gm_iszMortarClassname;
/*string_t CNPC_Dalek_Base::gm_iszFloorTurretClassname;
string_t CNPC_Dalek_Base::gm_iszGroundTurretClassname;
string_t CNPC_Dalek_Base::gm_iszShotgunClassname;
string_t CNPC_Dalek_Base::gm_iszRollerMineClassname;*/

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------

bool CNPC_Dalek_Base::CreateBehaviors()
{
#ifdef HL2_EPISODIC
...
bool CNPC_Dalek_Base::StartSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event, CChoreoActor *actor, CBaseEntity *pTarget)
{
	Assert(info);
...
bool CNPC_Dalek_Base::ProcessSceneEvent(CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
bool CNPC_Dalek_Base::ClearSceneEvent(CSceneEventInfo *info, bool fastKill, bool canceled)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
bool CNPC_Dalek_Base::CheckSceneEventCompletion(CSceneEventInfo *info, float currenttime, CChoreoScene *scene, CChoreoEvent *event)
{
	Assert(info);
	Assert(info->m_pScene);
	Assert(info->m_pEvent);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::Restore( IRestore &restore )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::ObjectCaps() 
{ 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::ShouldAlwaysThink() 
{ 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Disposition_t CNPC_Dalek_Base::IRelationType( CBaseEntity *pTarget )
{
	if ( !pTarget )
		return D_NU;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::IsSilentSquadMember() const
{
	if ( (const_cast<CNPC_Dalek_Base *>(this))->Classify() == CLASS_PLAYER_ALLY_VITAL && m_pSquad && MAKE_STRING(m_pSquad->GetName()) == GetPlayerSquadName() )
	{
		return true;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::GatherConditions()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::DoCustomSpeechAI( void )
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::PredictPlayerPush()
{
	CBasePlayer *pPlayer = AI_GetSinglePlayer();
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::BuildScheduleTestBits()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CSound *CNPC_Dalek_Base::GetBestSound( int validTypes )
{
	AISoundIter_t iter;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::QueryHearSound( CSound *pSound )
{
...
bool CNPC_Dalek_Base::QuerySeeEntity( CBaseEntity *pEntity, bool bOnlyHateOrFearIfNPC )
{
	CAI_BaseNPC *pOther = pEntity->MyNPCPointer(); 
	if ( pOther && ( pOther->GetState() == NPC_STATE_ALERT || GetState() == NPC_STATE_ALERT ||  pOther->GetState() == NPC_STATE_COMBAT || GetState() == NPC_STATE_COMBAT ))
	{
		return true;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::ShouldIgnoreSound( CSound *pSound )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::SelectSchedule()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::SelectScheduleDanger()
{
	if( HasCondition( COND_HEAR_DANGER ) )
	{
		CSound *pSound;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*int CNPC_Dalek_Base::SelectSchedulePriorityAction()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*int CNPC_Dalek_Base::SelectSchedulePlayerPush()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::IgnorePlayerPushing( void )
{
	if ( hl2_episodic.GetBool() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::SelectScheduleCombat()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::CanReload( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::ShouldDeferToFollowBehavior()
{
...
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsValidReasonableFacing( const Vector &vecSightDir, float sightDist )
{
	if( !GetActiveWeapon() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
		case TASK_SOUND_WAKE:
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::PrepareReadinessRemap( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::Activate( void )
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
Activity CNPC_Dalek_Base::TranslateActivityReadiness( Activity activity )
{
	// If we're in an actbusy, we don't want to mess with this
	if ( m_ActBusyBehavior.IsActive() )
		return activity;

	if ( m_bReadinessCapable && ( GetReadinessUse() == AIRU_ALWAYS ) )
	{
...
//-----------------------------------------------------------------------------
Activity CNPC_Dalek_Base::NPC_TranslateActivity( Activity activity )
{
...
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::HandleAnimEvent( animevent_t *pEvent )
{
/*#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
int CNPC_Dalek_Base::GetSoundInterests()
{
	return	SOUND_WORLD				|
			SOUND_COMBAT			|
			SOUND_PLAYER			|
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::Touch( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::ModifyOrAppendCriteria( AI_CriteriaSet& set )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::AddReadiness( float flAdd, bool bOverrideLock )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::SubtractReadiness( float flSub, bool bOverrideLock )
{
 	if( IsReadinessLocked() && !bOverrideLock )
		return;
...
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::AllowReadinessValueChange( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::SetReadinessValue( float flSet )
{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::SetReadinessLevel( int iLevel, bool bOverrideLock, bool bSlam )
{
	if( IsReadinessLocked() && !bOverrideLock )
		return;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int	CNPC_Dalek_Base::GetReadinessLevel()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::UpdateReadiness()
{
	...
	if ( !IsInScriptedReadinessState() )
	{
 	...
 	if( ai_debug_readinessdalek.GetBool() && AI_IsSinglePlayer() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_Dalek_Base::GetReadinessDecay()
{
	return ai_readinessdalek_decay.GetFloat();
}

//-----------------------------------------------------------------------------
// Passing NULL to clear the aim target is acceptible.
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::SetAimTarget( CBaseEntity *pTarget )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::StopAiming( char *pszReason )
{
#if 0
	if( pszReason )
	{	
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::HasActiveLookTargets(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::ClearLookTarget(CBaseEntity *pTarget)
{
...
//-----------------------------------------------------------------------------
float CNPC_Dalek_Base::PickLookTarget(bool bExcludePlayers, float minTime, float maxTime)
{
	return PickLookTarget(m_randomLookQueue, bExcludePlayers, minTime, maxTime);
}

float CNPC_Dalek_Base::PickLookTarget(CAI_InterestTarget &queue, bool bExcludePlayers, float minTime, float maxTime)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define COMPANION_MAX_LOOK_TIME	6.0f
#define COMPANION_MIN_LOOK_TIME	2.4f
#define COMPANION_MAX_TACTICAL_TARGET_DIST	1800.0f // 150 feet

bool CNPC_Dalek_Base::PickTacticalLookTarget(AILookTargetArgs_t *pArgs)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::ValidEyeTargetDalek(const Vector &lookTargetPos)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::ValidHeadTargetDalek(const Vector &lookTargetPos)
{
...
//-----------------------------------------------------------------------------
float CNPC_Dalek_Base::HeadTargetValidity(const Vector &lookTargetPos)
{
	Vector vFacing = BodyDirection3D();
...
bool CNPC_Dalek_Base::PickRandomLookTarget(AILookTargetArgs_t *pArgs)
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::MakeRandomLookTarget(AILookTargetArgs_t *pArgs, float minTime, float maxTime)
{
...
void CNPC_Dalek_Base::StartTaskRangeAttack1(const Task_t *pTask)
{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::AddLookTarget(CBaseEntity *pTarget, float flImportance, float flDuration, float flRamp)
{
	m_lookQueue.Add(pTarget, flImportance, flDuration, flRamp);
}


void CNPC_Dalek_Base::AddLookTarget(const Vector &vecPosition, float flImportance, float flDuration, float flRamp)
{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::MaintainLookTargets(float flInterval)
{
	int i;
	...
	if (m_lookQueue.Count() != 0 || m_syntheticLookQueue.Count() != 0 || (GetState() == NPC_STATE_SCRIPT))
	{
	...
	bool bValidHeadTargetDalek = false;
	...
	for (i = active.Count() - 1; i >= 0; i--)
	{
		...
		{
			// E3 Hack
			if (ValidEyeTargetDalek(active[i]->GetPosition()))
			{
	...
	if (!bFoundTarget && !ValidEyeTargetDalek(GetViewtarget()))
	{
		Vector right, up;
	...
	if (ai_debug_looktargetsdalek.GetInt() == 1 && (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
	{
...
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::FindNewAimTarget()
{
	if( GetEnemy() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::OnNewLookTarget()
{
	if( ai_new_aimingdalek.GetBool() )
	{
		if( GetLooktarget() )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::ShouldBeAiming() 
{
	if( !IsAllowedToAim() )
	{
		return false;
...
bool CNPC_Dalek_Base::IsAllowedToAim()
{
	//if( !m_pSquad )
	//	return true;
	...
	{
		CNPC_Dalek_Base *pCompanion = dynamic_cast<CNPC_Dalek_Base*>(pSquadmate);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_Dalek_Base::GetAlternateMoveShootTarget()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsValidEnemy( CBaseEntity *pEnemy )
{
	if ( GetFollowBehavior().GetFollowTarget() && IsSniper( pEnemy ) )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsSafeFromFloorTurret( const Vector &vecLocation, CBaseEntity *pTurret )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CNPC_Dalek_Base::ShouldMoveAndShoot( void )
{
...
void CNPC_Dalek_Base::OnUpdateShotRegulator()
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
/*void CNPC_Dalek_Base::DecalTrace( trace_t *pTrace, char const *decalName )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CNPC_Dalek_Base::FCanCheckAttacks()
{
...
Vector CNPC_Dalek_Base::GetActualShootPosition( const Vector &shootOrigin )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
/*WeaponProficiency_t CNPC_Dalek_Base::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::Weapon_CanUse( CBaseCombatWeapon *pWeapon )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::ShouldLookForBetterWeapon()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
/*void CNPC_Dalek_Base::PickupWeapon( CBaseCombatWeapon *pWeapon )
{
...
CUtlVector<AI_EnemyInfo_t *>	g_MultiCoverSearchEnemiesDalek;
CNPC_Dalek_Base *			g_pMultiCoverSearcher;

//-------------------------------------

int __cdecl MultiCoverCompareDalek( AI_EnemyInfo_t * const *ppLeft, AI_EnemyInfo_t * const *ppRight )
{
	...
	bool bLeftIsSpecial = ( CNPC_Dalek_Base::IsMortar( pLeft->hEnemy ) || CNPC_Dalek_Base::IsSniper( pLeft->hEnemy ) );
	bool bRightIsSpecial = ( CNPC_Dalek_Base::IsMortar( pLeft->hEnemy ) || CNPC_Dalek_Base::IsSniper( pLeft->hEnemy ) );
...
void CNPC_Dalek_Base::SetupCoverSearch( CBaseEntity *pEntity )
{
	/*if ( IsTurret( pEntity ) )
			...
			if ( !pEntity || GetEnemies()->NumEnemies() > 1 )
			{
				...
				for ( AI_EnemyInfo_t *pEnemyInfo = GetEnemies()->GetFirst(&iter); pEnemyInfo != NULL; pEnemyInfo = GetEnemies()->GetNext(&iter) )
				{
					...
					if ( pEnemy )
					{
						...
						g_MultiCoverSearchEnemiesDalek.AddToTail( pEnemyInfo );
					}
				}

				if ( g_MultiCoverSearchEnemiesDalek.Count() == 0 )
				{
					gm_bFindingCoverFromAllEnemies = false;
				}
				else if ( gm_bFindingCoverFromAllEnemies )
				{
					g_MultiCoverSearchEnemiesDalek.Sort( MultiCoverCompareDalek );
					Assert( g_MultiCoverSearchEnemiesDalek[0]->hEnemy == GetEnemy() );
				}
			}
		}
	}*/
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::CleanupCoverSearch()
{
	gm_fCoverSearchType = CT_NORMAL;
	g_MultiCoverSearchEnemiesDalek.RemoveAll();
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::FindCoverPos( CBaseEntity *pEntity, Vector *pResult)
{
	AI_PROFILE_SCOPE(CNPC_Dalek_Base_FindCoverPos);
...
bool CNPC_Dalek_Base::FindCoverPosInRadius( CBaseEntity *pEntity, const Vector &goalPos, float coverRadius, Vector *pResult )
{
	AI_PROFILE_SCOPE(CNPC_Dalek_Base_FindCoverPosInRadius);
...
bool CNPC_Dalek_Base::FindCoverPos( CSound *pSound, Vector *pResult )
{
	AI_PROFILE_SCOPE(CNPC_Dalek_Base_FindCoverPos);
...
bool CNPC_Dalek_Base::FindMortarCoverPos( CSound *pSound, Vector *pResult )
{
	bool result = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsCoverPosition( const Vector &vecThreat, const Vector &vecPosition )
{
	if ( gm_bFindingCoverFromAllEnemies )
	{
		for ( int i = 0; i < g_MultiCoverSearchEnemiesDalek.Count(); i++ )
		{
			// @TODO (toml 07-27-04): Should skip checking points near already checked points
			AI_EnemyInfo_t *pEnemyInfo = g_MultiCoverSearchEnemiesDalek[i];
			Vector testPos;
			CBaseEntity *pEnemy = pEnemyInfo->hEnemy;
			if ( !pEnemy )
				continue;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsMortar( CBaseEntity *pEntity )
{
	if ( !pEntity )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsSniper( CBaseEntity *pEntity )
{
	if ( !pEntity )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::IsTurret( CBaseEntity *pEntity )
{
	if ( !pEntity )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsGunship( CBaseEntity *pEntity )
{
	if( !pEntity )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Dalek_Base::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	if( info.GetAttacker() )
	{
		bool bIsEnvFire;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::OnFriendDamaged( CBaseCombatCharacter *pSquadmate, CBaseEntity *pAttackerEnt )
{
	AI_PROFILE_SCOPE( CNPC_Dalek_Base_OnFriendDamaged );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Dalek_Base::IsValidMoveAwayDest( const Vector &vecDest )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::FValidateHintType( CAI_Hint *pHint )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::ValidateNavGoal()
{
	bool result;
...
bool CNPC_Dalek_Base::OverrideMove( float flInterval )
{
	...
	if ( !overrode && GetNavigator()->GetGoalType() != GOALTYPE_NONE )
	{
		...
		while( ( pEntity = OverrideMoveCacheDalek_FindTargetsInRadius( pEntity, GetAbsOrigin(), AVOID_TEST_DIST ) ) != NULL )
		{
			// Handle each type
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_Dalek_Base::GetIdealSpeed() const
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*float CNPC_Dalek_Base::GetIdealAccel() const
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
...
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::ShouldAlwaysTransition( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputOutsideTransition( inputdata_t &inputdata )
{
	if ( !AI_IsSinglePlayer() )
		return;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputSetReadinessPanic( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputSetReadinessStealth( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputSetReadinessLow( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputSetReadinessMedium( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputSetReadinessHigh( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputLockReadiness( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::LockReadiness( float duration )
{
	if ( duration == -1.0f )
	{
...
//-----------------------------------------------------------------------------
void CNPC_Dalek_Base::UnlockReadiness( void )
{
	// Set to the past
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::ShouldDeferToPassengerBehavior( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::CanEnterVehicle( void )
{
	return true;
}*/
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::CanExitVehicle( void )
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::EnterVehicle( CBaseEntity *pEntityVehicle, bool bImmediately )
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputEnterVehicle( inputdata_t &inputdata )
{
...
// Input  : &inputdata - contains the entity name of the vehicle to enter
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputEnterVehicleImmediately( inputdata_t &inputdata )
{
	CBaseEntity *pEntity = FindNamedEntity( inputdata.value.String() );
...
// Purpose: 
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputExitVehicle( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputCancelEnterVehicle( inputdata_t &inputdata )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::ExitVehicle( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::IsInAVehicle( void ) const
{
...
//-----------------------------------------------------------------------------
/*IServerVehicle *CNPC_Dalek_Base::GetVehicle( void )
{
	if ( IsInAVehicle() )
	{
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
/*CBaseEntity *CNPC_Dalek_Base::GetVehicleEntity( void )
{
	if ( IsInAVehicle() )
	{
		CPropVehicleDriveable *pDriveableVehicle = m_PassengerBehavior.GetTargetVehicle();
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::UpdateEfficiency( bool bInPVS )
{ 
...
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::CanRunAScriptedNPCInteraction( bool bForced = false )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::IsAllowedToDodge( void )
{
	// TODO: Allow this but only for interactions who stem from being in a vehicle?
	if ( IsInAVehicle() )
		return false;
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputEnableAlwaysTransition( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputDisableAlwaysTransition( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputEnableWeaponPickup( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputDisableWeaponPickup( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
/*void CNPC_Dalek_Base::InputGiveWeapon( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
void CNPC_Dalek_Base::InputClearAllOuputs( inputdata_t &inputdata )
{
...
//			&info - How they died
//-----------------------------------------------------------------------------
/*oid CNPC_Dalek_Base::OnPlayerKilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
/*bool CNPC_Dalek_Base::IsNavigationUrgent( void )
{
	bool bBase = BaseClass::IsNavigationUrgent();

	// Consider follow & assault behaviour urgent
	if ( !bBase && (m_FollowBehavior.IsActive() || ( m_AssaultBehavior.IsRunning() && m_AssaultBehavior.IsUrgent() )) && Classify() == CLASS_DALEK ) 
	{
...
AI_BEGIN_CUSTOM_NPC( dalek_base, CNPC_Dalek_Base )
...
class COverrideMoveCacheDalek : public IEntityListener
{
public:
...
COverrideMoveCacheDalek g_OverrideMoveCacheDalek;
COverrideMoveCacheDalek *OverrideMoveCacheDalek( void ) { return &g_OverrideMoveCacheDalek; }

CBaseEntity *OverrideMoveCacheDalek_FindTargetsInRadius( CBaseEntity *pFirstEntity, const Vector &vecOrigin, float flRadius )
{
	return g_OverrideMoveCacheDalek.FindTargetsInRadius( pFirstEntity, vecOrigin, flRadius );
}

/*void OverrideMoveCacheDalek_ForceRepopulateList( void )
{
	g_OverrideMoveCacheDalek.ForceRepopulateList();
}*/

void OverrideMoveCacheDalek_LevelInitPreEntity( void )
{
	g_OverrideMoveCacheDalek.LevelInitPreEntity();
}

void OverrideMoveCacheDalek_LevelShutdownPostEntity( void )
{
	g_OverrideMoveCacheDalek.LevelShutdownPostEntity();
}
