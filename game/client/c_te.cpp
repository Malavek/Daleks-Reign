...
void TE_DalekGunExplosion(IRecipientFilter& filter, float delayt,
	const Vector &pos, const Vector &dir, int type );
...
public:
	...
	virtual void DalekGunExplosion( IRecipientFilter& filter, float delay,
				const Vector &pos, const Vector &dir, int type )
	{
		if ( !SuppressTE( filter ) )
		{
			TE_DalekGunExplosion( filter, delay, pos, dir, type );
		}
	}
