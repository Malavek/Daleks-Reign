...
//			type - 
//-----------------------------------------------------------------------------
void FX_DalekGunExplosion(const Vector &pos, const Vector &dir, int type)
{
	Vector	vDir;
	...
	// XBox version
	//
	CSmartPtr<CTrailParticles> pSparkEmitter = CTrailParticles::Create("FX_DalekGunExplosion");
	if (pSparkEmitter == NULL)
	{
		Assert(0);
		return;
...
class C_TEDalekGunExplosion : public C_TEParticleSystem
{
public:
	DECLARE_CLASS(C_TEDalekGunExplosion, C_TEParticleSystem);
	DECLARE_CLIENTCLASS();

	C_TEDalekGunExplosion();
	virtual		~C_TEDalekGunExplosion();
...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEDalekGunExplosion, DT_TEDalekGunExplosion, CTEDalekGunExplosion)
RecvPropInt(RECVINFO(m_nType)),
RecvPropVector(RECVINFO(m_vecDirection)),
END_RECV_TABLE()
...
C_TEDalekGunExplosion::C_TEDalekGunExplosion()
{
}
...
// Input  : bNewEntity - whether or not to start a new entity
//-----------------------------------------------------------------------------
void C_TEDalekGunExplosion::PostDataUpdate(DataUpdateType_t updateType)
{
	FX_DalekGunExplosion(m_vecOrigin, m_vecDirection, m_nType);
}
...
//			type - 
//-----------------------------------------------------------------------------
void TE_DalekGunExplosion(IRecipientFilter& filter, float delay, const Vector &pos, const Vector &dir, int type)
{
	FX_DalekGunExplosion(pos, dir, type);
}