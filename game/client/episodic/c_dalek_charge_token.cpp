...
class C_NPC_Dalek : public C_AI_BaseNPC
{
	DECLARE_CLASS(C_NPC_Dalek, C_AI_BaseNPC);
	DECLARE_CLIENTCLASS();
...
IMPLEMENT_CLIENTCLASS_DT(C_NPC_Dalek, DT_NPC_Dalek, CNPC_Dalek)
...
// Input  : updateType - 
//-----------------------------------------------------------------------------
void C_NPC_Dalek::OnDataChanged(DataUpdateType_t updateType)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_NPC_Dalek::ClientThink(void)
{
...
//-----------------------------------------------------------------------------
void C_NPC_Dalek::ReceiveMessage(int classID, bf_read &msg)
{
	...
	case VORTFX_ZAPBEAM:
	{
		...
		CNewParticleEffect *pEffect = ParticleProp()->Create("Weapon_Combine_Ion_Cannon_Beam", PATTACH_POINT_FOLLOW, nAttachment);
		if (pEffect)
		{
...
class C_DalekChargeToken : public C_BaseEntity
{
	DECLARE_CLASS(C_DalekChargeToken, C_BaseEntity);
	DECLARE_CLIENTCLASS();
...
void RecvProxy_DalekFadeOutDuration(const CRecvProxyData *pData, void *pStruct, void *pOut)
{
	C_DalekChargeToken *pVortToken = (C_DalekChargeToken *)pStruct;
...
IMPLEMENT_CLIENTCLASS_DT(C_DalekChargeToken, DT_DalekChargeToken, CDalekChargeToken)
RecvPropBool(RECVINFO(m_bFadeOut)),
END_RECV_TABLE()

void C_DalekChargeToken::UpdateOnRemove(void)
{
	if (m_hEffect)
	{
...
//-----------------------------------------------------------------------------
void C_DalekChargeToken::NotifyShouldTransmit(ShouldTransmitState_t state)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekChargeToken::OnDataChanged(DataUpdateType_t type)
{
	if (m_bFadeOut)
	{
		if (m_hEffect)
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekChargeToken::ClientThink(void)
{
	//
	// -- DLight 
	//
...
class C_DalekEffectDispel : public C_BaseEntity
{
	DECLARE_CLASS(C_DalekEffectDispel, C_BaseEntity);
	DECLARE_CLIENTCLASS();
...
void RecvProxy_DispelDalekFadeOutDuration(const CRecvProxyData *pData, void *pStruct, void *pOut)
{
	C_DalekEffectDispel *pVortToken = (C_DalekEffectDispel *)pStruct;
	Assert(pOut == &pVortToken->m_flFadeOutTime);
...
IMPLEMENT_CLIENTCLASS_DT(C_DalekEffectDispel, DT_DalekEffectDispel, CDalekEffectDispel)
RecvPropBool(RECVINFO(m_bFadeOut)),
END_RECV_TABLE()

void C_DalekEffectDispel::UpdateOnRemove(void)
{
	if (m_hEffect)
	{
		m_hEffect->StopEmission();
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekEffectDispel::OnDataChanged(DataUpdateType_t type)
{
	if (m_bFadeOut)
	{
		if (m_hEffect)
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekEffectDispel::NotifyShouldTransmit(ShouldTransmitState_t state)
{
	BaseClass::NotifyShouldTransmit(state);
...
//-----------------------------------------------------------------------------
bool C_DalekEffectDispel::SetupEmitters(void)
{
	m_pDLight = NULL;
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekEffectDispel::ClientThink(void)
{
	if (m_pDLight != NULL)
	{
		m_pDLight->origin = GetAbsOrigin();
...
void RecvProxy_LampDalekFadeOutDuration(const CRecvProxyData *pData, void *pStruct, void *pOut)
{
	C_DalekEffectLamp *pVortToken = (C_DalekEffectLamp *)pStruct;
	Assert(pOut == &pVortToken->m_flFadeOutTime);
...
IMPLEMENT_CLIENTCLASS_DT(C_DalekEffectLamp, DT_DalekEffectLamp, CDalekEffectLamp)
RecvPropBool(RECVINFO(m_bFadeOut)),
END_RECV_TABLE()

void C_DalekEffectLamp::UpdateOnRemove(void)
{
	if (m_hEffect)
	{
		m_hEffect->StopEmission();
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekEffectLamp::OnDataChanged(DataUpdateType_t type)
{
	if (m_bFadeOut)
	{
		if (m_hEffect)
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekEffectLamp::NotifyShouldTransmit(ShouldTransmitState_t state)
{
	BaseClass::NotifyShouldTransmit(state);
...
// Purpose: Create our emitter
//-----------------------------------------------------------------------------
bool C_DalekEffectLamp::SetupEmitters(void)
{
	m_pDLight = NULL;
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_DalekEffectLamp::ClientThink(void)
{
	if (m_pDLight != NULL)
	{
		m_pDLight->origin = GetAbsOrigin();
...
// Purpose: 
//-----------------------------------------------------------------------------
void DalekDispelCallback(const CEffectData &data)
{
	// Kaboom!
...
DECLARE_CLIENT_EFFECT("VortDispel", DalekDispelCallback);

//-----------------------------------------------------------------------------
// Purpose: Used for emissive lightning layer on vort
//-----------------------------------------------------------------------------
class CDalekEmissiveProxy : public CEntityMaterialProxy
{
public:
	CDalekEmissiveProxy(void);
	virtual				~CDalekEmissiveProxy(void);
...
//-----------------------------------------------------------------------------
CDalekEmissiveProxy::CDalekEmissiveProxy(void)
{
...
//-----------------------------------------------------------------------------
bool CDalekEmissiveProxy::Init(IMaterial *pMaterial, KeyValues* pKeyValues)
{
	Assert(pMaterial);
...
//-----------------------------------------------------------------------------
void CDalekEmissiveProxy::OnBind(C_BaseEntity *pEnt)
{
	C_NPC_Dalek *pVort = dynamic_cast<C_NPC_Dalek *>(pEnt);
	...
	if (pVort)
	{
		...
		// C_NPC_Dalek::ClientThink() function. 
		// 
		// pVort->m_flBlackFade
	}
	else
...
//-----------------------------------------------------------------------------
IMaterial *CDalekEmissiveProxy::GetMaterial()
{
	if (m_pMatEmissiveStrength != NULL)
...
EXPOSE_INTERFACE(CDalekEmissiveProxy, IMaterialProxy, "DalekEmissive" IMATERIAL_PROXY_INTERFACE_VERSION);
